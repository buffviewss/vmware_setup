...
try {
    ...
    Write-Host "Không thể lấy danh sách font. Kiểm tra kết nối mạng."
    exit 1
}
...
foreach ($font in $randomFonts) {
    ...
    if (-not $ttfUrl) {
        Write-Host "Không tìm thấy file TTF cho font $fontName, bỏ qua."
        continue
    }
    ...
    try {
        ...
        Write-Host "Tải font $fontName thất bại, bỏ qua."
        continue
    }
    ...
    try {
        ...
        Write-Host "Không thể copy font $fontName vào thư mục hệ thống."
        continue
    }
    ...
    try {
        ...
        Write-Host "Không thể đăng ký font $fontName vào registry."
        continue
    }

    Write-Host "Đã cài đặt font: $fontName"

    # Delay ngẫu nhiên để giống thao tác người thật
    Start-Sleep -Seconds (Get-Random -Minimum 2 -Maximum 6)
}

Write-Host "`nĐã cài đặt $fontCount font mới từ Google Fonts!"
