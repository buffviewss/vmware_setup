...
function Test-AdminRights {
    try {
        ...
        Write-Host "Đã xác nhận quyền quản trị."
    } catch {
        Write-Host "Lỗi khi kiểm tra quyền quản trị: $_"
        exit 1
    }
}
    ...
    Get-ChildItem $p -ErrorAction SilentlyContinue | ForEach-Object {
      ...
      if ($prop.DisplayName -like "Google Chrome*") {
        ...
        if ($uninst) {
          ...
          $exe,$arguments = $uninst -split '\.exe"\s*',2
          if ($exe -notmatch '\.exe"$') { $exe = $uninst } else { $exe = $exe + '.exe"' ; $arguments = $arguments.Trim() }
          $exe = $exe.Trim('"')
          # Thử tham số silent phổ biến
          $extra = ' --uninstall --multi-install --chrome --force-uninstall'
          Start-Process -FilePath $exe -ArgumentList ($arguments + $extra) -Wait -ErrorAction Stop
...
function Get-ChromeInstaller {
    ...
    Get-FileWithGdown -FileID $FileID -OutputPath $DownloadPathChrome
...
function Get-NekoboxInstaller {
    ...
    # Sử dụng gdown để tải file
    Get-FileWithGdown -FileID $FileID -OutputPath $DownloadPathNekoBox
...
function Expand-NekoboxArchive {
...
function Add-NekoboxToTaskbar {
    Write-Host "Đang tạo shortcut cho Nekobox..."

    try {
        $startMenuPath = "$env:APPDATA\Microsoft\Windows\Start Menu\Programs"
        $shortcutPath = "$startMenuPath\Nekobox.lnk"
        ...
        Write-Host "Đã tạo shortcut cho Nekobox trong Start Menu."
        Write-Host "Lưu ý: Bạn có thể tự pin vào Taskbar bằng cách right-click vào shortcut."
    } catch {
        Write-Host "Lỗi khi tạo shortcut cho Nekobox: $_"
        # Không exit vì đây không phải lỗi nghiêm trọng
    }
}
...
function Install-SelectedChrome {
    $chromeFileID = Select-ChromeVersion

    # Tải tệp Chrome từ Google Drive
    Get-ChromeInstaller -FileID $chromeFileID
    Uninstall-Chrome
    Install-Chrome
}
...
# Cài đặt Python và gdown nếu cần
...
Install-SelectedChrome

# Khóa cập nhật tự động của Chrome
Disable-AutoUpdateChrome

# Tải, giải nén và cài đặt Nekobox
Get-NekoboxInstaller -FileID $NekoBoxFileID
Expand-NekoboxArchive
Install-Nekobox

# Thiết lập Nekobox tự động mở khi khởi động Windows
Set-AutoStart

# Tạo shortcut cho Nekobox
